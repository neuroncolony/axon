...
window.AXON = (() => {
  ...
  function renderNav() {
    ...
    nav.innerHTML = `<div class="wrap"><div class="pill"><a class="brand" href="${href('/')}"><span class="mark"></span><span class="wm-text">${BRAND.name}</span></a><nav class="nav-links">${NAV.map(([p,l])=>`<a href="${href(p)}" class="${path.startsWith(p)?'active':''}">${l}</a>`).join('')}</nav><span class="spacer"></span>${state.address ? `<button class="btn sm ghost mono" id="acct">${short(state.address)}</button>` : `<button class="btn sm ghost" id="connect">Connect wallet</button>`}<a class="btn sm accent" href="${href('/launch')}">Launch a coin</a><button class="btn sm ghost nav-burger" id="burger" aria-label="menu">&#9776;</button></div></div><div class="nav-mobile" id="navm">${NAV.map(([p,l])=>`<a href="${href(p)}" class="${path.startsWith(p)?'active':''}">${l}</a>`).join('')}</div>`;
