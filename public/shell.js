...
window.AXON = (() => {
  ...
  function renderNav() {
    ...
    nav.innerHTML = `<div class="wrap"><a class="brand" href="${href('/')}"><span class="mark"></span>${BRAND.name}</a><nav class="nav-links">${NAV.map(([p,l])=>`<a href="${href(p)}" class="${path.startsWith(p)?'active':''}">${l}</a>`).join('')}</nav><span class="spacer"></span><a class="btn sm accent" href="${href('/launch')}">+ Create</a>${state.address ? `<button class="btn sm ghost mono" id="acct">${short(state.address)}</button>` : `<button class="btn sm ghost" id="connect">Connect wallet →</button>`}<button class="btn sm ghost nav-burger" id="burger" aria-label="menu">≡</button></div><div class="nav-mobile" id="navm">${NAV.map(([p,l])=>`<a href="${href(p)}">${l}</a>`).join('')}<a href="${href('/launch')}">+ Create</a></div>`;
